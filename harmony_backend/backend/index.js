...
app.post("/api/eventParticipants/:id/skipped/:matchId", async (req, res) => {
  try {
    const eventId = String(req.query.eventId || "").trim();
    const participantRouteId = String(req.params.id || "").trim();
    const matchId = String(req.params.matchId || "").trim();
    const reason = String(req.body?.reason || "").trim();

    if (!eventId || !participantRouteId || !matchId) {
      return res.status(400).json({ message: "Missing required parameters" });
    }

    if (!["not_relevant", "not_available"].includes(reason)) {
      return res.status(400).json({ message: "Invalid skip reason" });
    }

    const resource = await getParticipantDocByRouteId(participantRouteId, eventId);

    if (!resource) {
      return res.status(404).json({ message: "Participant not found" });
    }

    const skipped = Array.isArray(resource.skipped) ? resource.skipped : [];
    const skippedDetails = Array.isArray(resource.skippedDetails)
      ? resource.skippedDetails
      : [];
    ...
    const existingIndex = skippedDetails.findIndex(
      item => String(item.matchId) === matchId
    );
    ...
    if (existingIndex >= 0) {
      skippedDetails[existingIndex] = skipItem;
    } else {
      skippedDetails.push(skipItem);
    }

    resource.skipped = skipped;
    resource.skippedDetails = skippedDetails;
    ...
    res.json({
      message: "Skipped saved successfully",
      skipped: updated.skipped || [],
      skippedDetails: updated.skippedDetails || []
    });
  } catch (error) {
    res.status(500).json({
      message: "Save skipped failed",
      error: error.message
    });
  }
});
